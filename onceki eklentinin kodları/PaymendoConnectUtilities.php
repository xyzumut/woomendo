<?php
    ...
    class PaymendoConnectUtilities{
        ...
        public function handleLogin($response, $isRefreshToken = false){
            ...
            update_option( 'woomendo_ayar_token_expires_in', $response['expires_in']+time()); # access_token'ın öleceği zamanı belirlediğimiz yer
            $this->get_paymendo_connection()->set_paymendo_expiresIN(get_option('woomendo_ayar_token_expires_in'));
            update_option( 'woomendo_ayar_access_token', $response['access_token']);
            $this->get_paymendo_connection()->set_paymendo_accessToken($response['access_token']);
            update_option( 'woomendo_ayar_refresh_token', $response['refresh_token']);
            $this->get_paymendo_connection()->set_paymendo_refreshToken($response['refresh_token']);
            update_option( 'woomendo_ayar_isLogin', 'true');
        ...
        public function loginWithPassword(){
            $response = $this->paymendoRequest(
                'POST', 
                $this->get_paymendo_connection()::woomendo_api_login_url,
                (object) [
                    'data' => [
                        'attributes' => [
                            'grant_type'=> 'password',
        ...
        public function paymendoRequest($method = 'GET', $url = '', $data = array(), $refresh=false) {
            ...
            $loginSwitch = PaymendoConnection::woomendo_api_login_url === $url ? true : false;
            ...
            if( strpos($url, PaymendoConnection::woomendo_api_login_url) === false ) {
        ...
        public function createOrder($data){
            $url = $this->get_paymendo_connection()::woomendo_get_post_api_url;
            $data = (object) [
                'data' => [
                    'attributes' => [
                        'amount'=>$data['amount'],
        ...
        public function getOrder($id=null, $page_size=8, $page_number=0){
            $url = $this->get_paymendo_connection()->get_paymendo_baseURL().$this->get_paymendo_connection()::woomendo_get_post_api_url;
            if (!is_null($id)) {
                $url = $url.'/'.$id;
            }
            else {
        ...
        public function makePayment($credit_card_data){
            $url = PaymendoConnection::woomendo_payment_api_url;
            $data = (object) [
                'data' => [
                    'attributes' => [
